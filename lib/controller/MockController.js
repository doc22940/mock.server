...
MockController.prototype = extend(MockController.prototype, {
	...
	getResponseOptions: function (req, res) {
		...
		var dir = this._processOptionsFallback(req, this._findFolder(path, this.options) + '/' + method + '/', this.options);
	...
	_processOptionsFallback: function (req, dir, options) {
		if (!fs.existsSync(dir) && req.method == 'OPTIONS') {
			if(this.options.optionsFallbackPath) {
				if(this.options.optionsFallbackPath.slice(-1) == '/') {
					return this.options.optionsFallbackPath;
				} else {
					return this.options.optionsFallbackPath + '/';
				}
			}
