...
var Utils = require('./Utils'),
	...
	DTOToResponseFuncConverter = require('./DTOToResponseFuncConverter'),
...
SwaggerPathMethod.prototype = extend(SwaggerPathMethod.prototype, {
	...
	init: function (name, methodObject, options) {
		...
		this._responseFakerData = undefined;
	...
	setDefinitions: function (swaggerDefinitions) {
		...
		this._responseFakerData = this._mapFakerData();
	},
	...
	write: function (path) {
		...
		var dir = path + '/' + this.getName(),
			...
			fileFaker = dir + '/mock/success-faker.json';
		...
		if (!existDir || (existDir && this._options.overwriteExistingDescriptions)) {
			...
			if (this._options.createFakerFile) {
				this.writeFile(fileFaker, this.getFileFakerData());
			}
	...
	/**
	 * @method getFileFakerData
	 * @returns {String}
	 * @public
	 */
	getFileFakerData: function () {

		var data = JSON.stringify(this.getResponseFakerData(), null, 2),
			responseFuncHeader = this.readFile(__dirname + '/../src/templates/response_func_header.tpl');

		data = data.replace(/(")(_getArray\([^"]*)(")/g, '<%-$2%>');
		data = data.replace(/(")(<%-_getRef\([^"]*)(")/g, '$2');
		data = data.replace(/(<%-)(_getRef\([^\)]*\))(;%>)/g, '$1JSON.stringify($2, null, 2)$3');
		responseFuncHeader = responseFuncHeader.replace(/<%funcPath%>/g, this._options.responseFuncPath.replace(this._options.dirName, ''));

		return responseFuncHeader + data;
	},
	...
	/**
	 * @method _mapFakerData
	 * @returns {Object}
	 * @private
	 */
	_mapFakerData: function () {
		...
		return dtoToResponseFuncConverter.get();
	}
