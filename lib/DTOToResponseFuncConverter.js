...
DTOToResponseFuncConverter.prototype = extend(DTOToResponseFuncConverter.prototype, {
	...
	_mapUnknown: function (data, key) {
		...
		if (this._rules[key]) {
			return this._mapRule(this._rules[key]);
		}
	...
	_mapRule: function (rule) {
		...
		if (rule instanceof Array) {
			return this._mapArray(rule);
		}

		if (rule.faker) {
			if (rule.fakerArgs) {
				return this._getFaker(rule.faker, JSON.stringify(rule.fakerArgs));
			}

			return this._getFaker(rule.faker, '');
		} else {
			return this._mapUnknown(rule, '');
		}
