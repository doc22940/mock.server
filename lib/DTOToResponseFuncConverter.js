...
DTOToResponseFuncConverter.prototype = extend(DTOToResponseFuncConverter.prototype, {
	...
	_mapUnknown: function (data, key, path) {
		...
		if (this._rules[this._name] && path.length < 1) {
			this._mapRuleDTO(this._rules[this._name], data);
		}

		if (this._rules[key]) {
			ruleMapObj = this._mapRule(this._rules[key], data, path);
			if (ruleMapObj.valid) {
				return ruleMapObj.rule;
			}
		}

		if (path.length > 1 && this._rules[path.join('.')]) {
			ruleMapObjParent = this._mapRule(this._rules[path.join('.')], data, path);
			if (ruleMapObjParent.valid) {
				return ruleMapObjParent.rule;
			}
		}
	 ...
	 * @private
	 */
	_mapRule: function (rule, data, path) {
		...
		if (rule.faker) {
			...
			return {
				rule: this._mapUnknown(rule, '', path),
				valid: true
			};
		}
	 ...
	 * @private
	 */
	_mapRuleDTO: function (rule, data) {

		if (this.typeOf(rule) !== 'object' || this.typeOf(data) !== 'object') {
			return;
		}

		this.forIn(rule, function (key, value) {
			if (data[key] && this.typeOf(data[key] === this.typeOf(value))) {
				data[key] = value;
			}
		}.bind(this));
	},
