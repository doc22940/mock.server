...
DTOToResponseFuncConverter.prototype = extend(DTOToResponseFuncConverter.prototype, {
	...
	_mapReference: function (refString) {
		var name = refString.replace('$ref-', ''),
			fullName = responsePreKey + name,
			path = this._getFuncPath(),
			_require = 'require(__dirname + \'/' + path + '/' + name + '.js\')';

		if (this._isEjs) {
			return '<%-_getRef(\'' + name + '\');%>';
		}

		_require = 'require(__dirname + \'' + '/' + name + '.js\')';

		return '<%>JSON.parse(' + _require + '.' + fullName + '(loopArr.slice()))<%>';
	},
	 ...
	 * @private
	 */
	_getFuncPath: function () {
		return this._options.responseFuncPath.replace(this._options.dirName, '').replace(/^\//, '');
	},
