 ...
 * libs are also reflected in the packages where the library is used.
...
function getLernaPackagesByLib(flowTypedLibName, onMatch) {
	...
	getLernaPackages(packagePath => {
		...
		const packageDependencies = Object.assign({}, packageData.dependencies, packageData.devDependencies);
...
function refreshFlowInLernaPackage(packagePath) {
	...
	if (!fs.existsSync(flowConfigFile)) {
		return;
	}
	try {
